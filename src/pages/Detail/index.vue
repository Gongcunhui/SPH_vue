            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSale in spuSaleAttrList" :key="spuSale.id">
                ...
                <dd
                  ...
                  v-for="(spuSaleAttr, index) in spuSale.spuSaleAttrValueList"
                  :key="index" 
                  @click="changeActive(spuSaleAttr,spuSale.spuSaleAttrValueList)"
                >
...
export default {
  ...
  methods:{
    changeActive(spu,spuArr){
      // 售卖属性的排他思想:点击只让自己 有类名（isChecked==1） 
      spuArr.forEach(spu => {
        spu.isChecked =0
      })
      spu.isChecked = 1
    ...
    // 加入购物车的回调
    async addShopCart(){
      // 1：发请求---将产品加入到数据库(服务器并没有返回数据，只需要知道添加成功还是失败)
      // 2：添加成功---跳转路由
      // 3：添加失败---给用户提示
      /* 需要判断加入购物车 成功还是 失败 */
    try {
      await this.$store.dispatch('detail/AddOrUpdateShopCart',{skuId:this.$route.params.id,skuNum:this.skuNum})
        // 成功 路由跳转,需要将产品品信息带给下一个路由(skuInfo以及skuNum)
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))  //复杂数据通过会话存储让下一级路由获取数据
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuNum}}) //简单数据query参数带给下一个路由
    } catch (error) {
      // 失败 提示信息
      alert(error.message)
    }

  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        >
          <li>
            <img :src="order.imgUrl" alt="" width="80px" height="80px" />
          </li>
          <li>
            <p>{{ order.skuName }}</p>
            <h4>7天无理由退货</h4>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        收货人：<span>{{ userDefaultAddressInfo.consignee }}</span>
        <span>{{ userDefaultAddressInfo.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    // 获取商品清单
    this.$store.dispatch("trade/getOrder");
  },
  computed: {
    ...mapState("trade", ["addressInfo", "orderInfo"]),
    // 最终的收货人信息
    userDefaultAddressInfo() {
      return this.addressInfo.find((i) => i.isDefault == 1) || {};
    },
  },
  methods: {
    // 点击高亮
    ...
    async commitOrder() {
      ...
      let data = {
        consignee: this.userDefaultAddressInfo.consignee,  //收件人名字
        consigneeTel: this.userDefaultAddressInfo.phoneNum, //收件人电话
        deliveryAddress: this.userDefaultAddressInfo.fullAddress,
        paymentWay: "ONLINE",
        orderComment: this.msg,
        orderDetailList: this.orderInfo.detailArrayList
      };
       let result = await this.$API.reqCommitOrder(tradeNo,data);
      //  提交订单成功
       if(result.code==200){
